...
from datasets.samplers import CategoriesSampler, CategoriesSampler_Semi
...
def main(config):
    ...
    n_batch = 200
    ep_per_batch = 4
    batch_sampler = CategoriesSampler_Semi(
            dataset.label, n_batch, n_way, n_shot, n_unlabel, n_query,
    ...
    for epoch in range(1, test_epochs + 1):
        for data, _ in tqdm(loader, leave=False):
            x_shot, x_unlabel, x_query = fs.split_shot_query_semi(
                    data.cuda(), n_way, n_shot, n_unlabel, n_query,
                    ep_per_batch=ep_per_batch)
